...
**Rendering.** A single `render(now)` runs every `requestAnimationFrame` tick and redraws everything from scratch: terrain → buildings → wind readout box → base → shield dome → defense drones → tank → trails → projectiles → particles → damage numbers. The wind indicator is drawn directly on the canvas (`drawWindBox`), not in the HTML HUD — it was deliberately moved there from a DOM element, so don't reintroduce a DOM wind readout without removing this one.
